...
function requestLocationForAddress() {
  requestLocationAutofill({
    ...
    onDebug: (info) => {
      if (info.coords) {
        console.debug('[Location] coords', { lat: info.coords.latitude, lon: info.coords.longitude });
      }
      if (info.result) {
        console.debug('[Location] reverse geocode', { status: info.result.status, raw: info.result.raw, label: info.result.label });
      }
    },
    onAddress: (address) => {
      console.debug('[Location] applying address', address);
      const draft = setBookingDraft({ address, locationConsent: true, locationType: 'home' });
      renderDraftPanels(draft);
      appendAssistant(t('chat.location.added', 'Added your location: {address}').replace('{address}', address));
      const input = document.getElementById('custom-reply');
      if (input) {
        input.value = address;
